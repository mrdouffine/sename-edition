...
export default function StripeContributionPaymentPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        if (!contributionId) {
          setError("contributionId manquant.");
          setLoading(false);
          return;
        }
          ...
          embeddedCheckoutPromise = stripe
            .initEmbeddedCheckout({
              fetchClientSecret: async () => {
                const response = await fetchWithAuth("/api/payments/contributions/stripe/embedded-session", {
                  method: "POST",
                ...
                if (!response.ok || !payload.data?.clientSecret) {
                  if (response.status === 401) {
                    clearAuthToken();
                    router.push(`/connexion?next=${encodeURIComponent(`/contribution/paiement/stripe?contributionId=${contributionId}`)}`);
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-4xl px-4 py-10 sm:px-6">
      ...
      <h1 className="mb-2 text-2xl font-black text-[#181810] sm:text-3xl">Contribution Stripe</h1>
      <p className="mb-6 text-sm text-[#6b6959]">Validez votre contribution via Stripe Embedded Checkout.</p>

      {loading ? <p className="text-sm text-[#6b6959]">Chargement du paiement...</p> : null}
      {error ? (
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
          <p>{error}</p>
          <Link href="/" className="mt-3 inline-block rounded-lg bg-primary px-4 py-2 font-bold uppercase">
            Retour
          </Link>
        </div>
      ) : null}
